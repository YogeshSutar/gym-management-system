...
// delete data from Check out plan table 

if (isset($_GET['delete_checkout_username'])) {
    // $updateusername=$_GET['un'];
    $delete_checkout_new_orderid = $_GET['delete_checkout_new_orderid'];
    $delete_checkout_username=$_GET['delete_checkout_username'];
    $delete_checkout_planname=$_GET['delete_checkout_planname'];
    $query = "DELETE FROM `checkoutplane` WHERE `username`='$delete_checkout_username' && `selected_plan`='$delete_checkout_planname' && `order_new_id`='$delete_checkout_new_orderid'";
    $data = mysqli_query($conn, $query);

    if ($data) {
        $_SESSION['status'] = "Check out Plan Record deleted successfully";
        $_SESSION['status_code'] = "success";
        // echo "<script>alert('Admin Record deleted successfully');</script>";
        // header("location:adminpage.php");
?>
        <meta http-equiv="refresh" content="0, URL= http://localhost/gym/admin_panel_users_check_plan.php">
<?php
    } else {
        $_SESSION['status'] = "No delete Ckeck out Plan record";
...
<head>
    <title>Users Check Out Plan Page</title>
                        ...
                        <div class="col" style="margin-right: -30px; padding-right: 0px; margin-left: 30px;">
                            <div class="card ms-5">
                                ...
                                <div class="card-header ">
                                    ...
                                    <h1 style="font-size: 60px; ">Check Out Plan Table </h1>
                                ...
                                <div class="card-body">
                                    <table class="table table-bordered text-center ">
                                        ...
                                        if (isset($_POST['filter_btn'])) {
                                            ...
                                            $query = "SELECT * FROM `checkoutplane` WHERE CONCAT(id,orderid,name,username,selected_plan,txn_amount,status,payment_type,datetime) LIKE '%$filter_value%'";
                                        ...
                                        ?>
                                                    <tr class="innerdetail" style="color: black;">
                                                        ...
                                                        <td><?php echo $data['order_new_id'] ?></td>
                                        ...
                                        <td>
                                            <a href="admin_panel_users_check_plan.php?delete_checkout_new_orderid='.$data['order_new_id'].'&
                                            delete_checkout_username=' . $data['username'] . '& delete_checkout_planname='.$data['selected_plan'].'" id="delete_user_detail" onclick="return check_delete_detail()" > <i class="fa-solid fa-trash"></i></a>
                                        </td>'
                                                        ?>
                                                    </tr>
                                                    <script>
